// skills/installer.js — Skill 安装管理 (OpenClaw-aligned)
const { spawn } = require('child_process');
const { execSync } = require('child_process');

/**
 * 检查命令是否存在 (POSIX-aligned: command -v)
...
async function installDependency(spec) {
  ...
  switch (kind) {
    case 'brew':
      ...
      break;
    case 'npm':
      // OpenClaw-aligned: --ignore-scripts for security
    ...
    case 'go':
      ...
      break;
    case 'uv':
      // OpenClaw-aligned: uv tool install (not uv pip install)
      cmd = `uv tool install ${pkg}`;
      description = `Installing ${pkg} via uv tool install`;
      break;
    default:
