...
async function streamChat({ messages, systemPrompt, config, requestId, tools, sessionId, wsIdentity, ctx, adapter, hooks = {} }) {
  ...
  const baseTimeout = (config.timeoutSeconds || 900) * 1000 // 15min base (was 10min)
  const timeoutMs = Math.max(baseTimeout, participantCount * 180_000) // 3min per participant
  ...
  async function _runRounds() {
    for (let round = 0; ; round++) {
      ...
      // Record orchestrator text produced in this round.
      // ConversationStream: write as independent DB message immediately (correct position).
      ...
      if (roundText) {
        ...
        flowSteps.push({ name: '__text__', output: roundText })
      }
